...
public class ButtonInput : MonoBehaviour
{
    public void NextPlayer(){
        ...
        GameManager.terrain_manager.ShowFogOfWar(GameManager.player_manager.player_list[GameManager.player_view], GameManager.map_size); // Shows Fog of War for all players
        Vector2 coordinates = GameManager.player_manager.player_list[GameManager.player_view].GetCity(0).GetColRow();
        HexTile hexTile = GameManager.map_generation.hex_list[(int) coordinates.x * (int) GameManager.map_size.y + (int) coordinates.y];
    ...
    public void DestroyFog(){
        GameManager.terrain_manager.DestroyFog();
    }
