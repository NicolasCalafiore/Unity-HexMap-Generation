        ...
        private bool MeetsRegionRequirements(List<List<float>> map_factors, Vector2 map_size){  // Checks if map meets region requirements
            ...
            foreach(List<float> row in map_factors){
                foreach(float factor in row){
                    if(factor == (float) RegionsEnums.HexRegion.Desert) desert_count++;
                    if(factor == (float) RegionsEnums.HexRegion.Tundra) tundra_count++;
                    if(factor == (float) RegionsEnums.HexRegion.Grassland) grassland_count++;
                    if(factor == (float) RegionsEnums.HexRegion.Plain) plains_count++;
                    if(factor == (float) RegionsEnums.HexRegion.Ocean) ocean_count++;
                    if(factor == (float) RegionsEnums.HexRegion.Highland) highlands_count++;
                    if(factor == (float) RegionsEnums.HexRegion.Jungle) jungle_count++;
                    if(factor == (float) RegionsEnums.HexRegion.Swamp) swamp_count++;
                }
            }
        ...
        private List<List<float>> CombineRegionFactors(List<List<float>> rain_map, List<List<float>> temperature_map, Vector2 map_size, List<List<float>> ocean_map, List<List<float>> river_map){
            ...
            for(int i = 0; i < map_size.x; i++){
                for(int j = 0; j < map_size.y; j++){
                    ...
                    if(ocean_map[i][j] == (int) LandEnums.LandType.Water || river_map[i][j] == (int) LandEnums.LandType.Water){
                        if(ocean_map[i][j] == (int) LandEnums.LandType.Water){
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Ocean;
                        }
                        else{
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.River;
                        }
                        continue;
                    }
                    ...
                    if(rain_map[i][j] <= .35){
                        if(temperature_map[i][j] <= .2){
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Tundra;
                        }
                        else if(temperature_map[i][j] <= .35){
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Highland;
                        }
                        else if(temperature_map[i][j] <= .4){
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Grassland;
                        }
                        else if(temperature_map[i][j] <= .6){
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Plain;
                        }
                        else if(temperature_map[i][j] <= 1){
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Desert;
                        }
                    }
                        ...
                        if(temperature_map[i][j] <= .3 && rain_map[i][j] <= .45){
                            ...
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Jungle;
                        }
                    }

                    else if(rain_map[i][j] <= 1){
                        if(temperature_map[i][j] <= .6){
                            map_factors[i][j] = (float) RegionsEnums.HexRegion.Swamp;
                        }
